...
func (*plainEvent) isEvent() bool { return true }

func (*plainEvent) setRoutingInfo(AbstractStateMachine, AbstractStateMachine) {}
...
func TestSendTo(t *testing.T) {
	tests := []struct {
		...
		{
			name: "does not alter events without routing metadata",
			setup: func(t *testing.T) (context.Context, *environment, *testStateMachine, *testStateMachine) {
				sender := newTestStateMachine(newTestState("sender"))
				recipient := newTestStateMachine(newTestState("recipient"))
				env := newTestEnvironment(sender, recipient)
			...
			wantSize: 1,
			validate: func(t *testing.T, queue []AbstractEvent, sender, recipient *testStateMachine) {
				if _, ok := queue[0].(*plainEvent); !ok {
					t.Fatalf("expected queued event to be *plainEvent, got %T", queue[0])
				}
			},
		},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			ctx, env, target, sender := tc.setup(t)
			SendTo(ctx, target, tc.event)

			queue := env.queue[target.id()]
			if len(queue) != tc.wantSize {
				t.Fatalf("expected %d events in queue, got %d", tc.wantSize, len(queue))
			}

			if tc.validate != nil {
				tc.validate(t, queue, sender, target)
				return
			}

			if queue[len(queue)-1] != tc.event {
				t.Fatalf("queued event mismatch: got %v, want %v", queue[len(queue)-1], tc.event)
			}
		})
	}
}
