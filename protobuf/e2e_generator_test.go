...
func TestGenerateE2ETest_Golden(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			switch tt.name {
			case "single_method_single_input":
				services = []ServiceTestCase{
					{
						Spec:           spec,
			...
			case "single_method_multiple_inputs":
				services = []ServiceTestCase{
					{
						Spec:           spec,
						ServicePackage: "github.com/example/proto/user",
			...
			mainTestContent, err := os.ReadFile(mainTestPath) //nolint:gosec // test file path
			if err != nil {
				t.Fatalf("Failed to read main_test.go: %v", err)
			}

			serviceTestPath := filepath.Join(tmpDir, "e2e_test_service_test.go")
			serviceTestContent, err := os.ReadFile(serviceTestPath) //nolint:gosec // test file path
			if err != nil {
			...
			if *update {
				...
				if err := os.WriteFile(goldenMainPath, mainTestContent, 0o600); err != nil {
					t.Fatalf("Failed to update golden main_test file: %v", err)
				}
				if err := os.WriteFile(goldenServicePath, serviceTestContent, 0o600); err != nil {
