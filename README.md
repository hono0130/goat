...
`result.HasViolation()` reports whether any rule was violated. `result.Violations` is a list of `Violation`, each with `Rule` (the violated rule), `Path` (the sequence of worlds leading to the violation), and `Loop` (the cycle, if the violation involves one). `result.Summary` contains `TotalWorlds` (how many worlds were explored) and `ExecutionTimeMs`.
