...
func TestMeetingRoomReservationWithoutExclusion(t *testing.T) {
	...
	if result.Summary.TotalWorlds != 12152 {
		t.Fatalf("expected TotalWorlds=12152, got %d", result.Summary.TotalWorlds)
	}
	...
	if v.Rule != "Always no-double-book" {
		t.Fatalf("expected rule %q, got %q", "Always no-double-book", v.Rule)
	}
	if v.Loop != nil {
	...
	expectedPath := []goat.WorldSnapshot{
		// [0] initial state
		{
		...
		// [5] server1 starts processing client0's request → DBSelect
		{
			StateMachines: []goat.StateMachineSnapshot{client0Idle, client1Idle, dbIdle, server1Idle, server2Idle},
		...
		// [6] server1 gets select result (not reserved), server2 starts processing
		{
			StateMachines: []goat.StateMachineSnapshot{client0Idle, client1Idle, dbIdle, server1Idle, server2Idle},
			QueuedEvents: []goat.EventSnapshot{
				exitClient(), transClient(), entryClient(), exitClient(), transClient(), entryClient(),
		...
		// [12] server2 starts processing client1, DBSelect for client1
		{
			StateMachines: []goat.StateMachineSnapshot{client0Idle, client1Idle, dbIdle, server1Processing, server2Idle},
			QueuedEvents: []goat.EventSnapshot{
				exitClient(), transClient(), entryClient(), exitClient(), transClient(), entryClient(),
		...
		// [19] DB now has reservations for both clients → double booking!
		{
	...
	if diff := cmp.Diff(expectedPath, v.Path); diff != "" {
		t.Errorf("violation path mismatch (-want +got):\n%s", diff)
	}
}
