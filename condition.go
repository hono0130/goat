...
// ConditionName identifies a condition.
type ConditionName string

// Condition represents a property that must hold true throughout
...
// condition holds for the given world state, and false otherwise.
...
func (f conditionFunc) Name() ConditionName {
	return f.name
}

func (f conditionFunc) Evaluate(w world) bool {
	return f.fn(w)
}

// BoolCondition creates a simple condition from a constant boolean value.
...
//
// Returns a Condition that can be used with Test() or WithConditions().
//
// Example:
//
//	alwaysPass := goat.BoolCondition(true)
//	alwaysFail := goat.BoolCondition(false)
func BoolCondition(b bool) Condition {
	return conditionFunc{fn: func(w world) bool { return b }}
}
...
// Example:
//
//	serverCond := goat.NewCondition(serverSM, func(sm *ServerStateMachine) bool {
//	    return sm.ConnectionCount <= sm.MaxConnections
//	})
func NewCondition[T AbstractStateMachine](sm T, check func(T) bool) Condition {
	id := sm.id()
	return conditionFunc{fn: func(w world) bool {
...
//         return goat.NewMultiCondition(func(machines goat.Machines) bool {
...
//	})
func NewMultiCondition(checkFunc func(Machines) bool, sms ...AbstractStateMachine) Condition {
	return conditionFunc{fn: func(w world) bool {
...
// Example:
//
//	cond := goat.NewCondition2(client, server, func(c *Client, s *Server) bool {
//	    return true
//	})
func NewCondition2[T1, T2 AbstractStateMachine](sm1 T1, sm2 T2, check func(T1, T2) bool) Condition {
	return NewMultiCondition(func(ms Machines) bool {
...
// Example:
//
//	cond := goat.NewCondition3(client, server, db, func(c *Client, s *Server, d *Database) bool {
//	    return true
//	})
func NewCondition3[T1, T2, T3 AbstractStateMachine](sm1 T1, sm2 T2, sm3 T3, check func(T1, T2, T3) bool) Condition {
	return NewMultiCondition(func(ms Machines) bool {
		m1, ok := GetMachine(ms, sm1)
		if !ok {
			return false
